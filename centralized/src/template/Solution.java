...
//	public Set<Solution> chooseNeighbors(int shuffleCount, Random random)
//	{
...
//	}
	...
	public Set<Solution> chooseNeighbors(int shuffleCount, Random random)
	{
		...
		for (int shuffleIt = 0; shuffleIt < shuffleCount; shuffleIt++)
		{
			List<CentralizedAction> possibleActionSet = new ArrayList<>(possibleActions);
			List<CentralizedAction> newPlan = new ArrayList<>(plan.getActionList().size());
			int newPlanWeight = 0;
			boolean feasible = true;
			
			for (int newPlanIdx = 0; newPlanIdx < plan.getActionList().size(); newPlanIdx++)
			{
			...
			{
				List<CentralizedPlan> newCentralizedPlanList = new ArrayList<>(centralizedPlanList);
				newCentralizedPlanList.set(randomIndex,
				                           new CentralizedPlan(centralizedPlanList.get(randomIndex).getInitialCity(),
				                                               newPlan));
				neighbors.add(new Solution(vehicleList, newCentralizedPlanList));
			}
