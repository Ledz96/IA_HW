...
public class HeuristicHTDAdv2AuctionTemplate implements AuctionBehavior
{
...
//	private long predictionCorrection = 0L;
	...
	private void initAgentMaps(int id)
	{
		...
		Set<Topology.City> ourCities = vehicles.stream().map(Vehicle::homeCity).collect(Collectors.toSet());
		cities.removeAll(ourCities);

		List<Vehicle> newVehicleList = new ArrayList<>();
		for (Vehicle vehicle : vehicles)
		{
			if (cities.isEmpty())
				cities.addAll(ourCities);
...
//		System.out.printf("minBidHistoryWindow: %s%n", minBidHistoryWindow);
	...
	private Solution findBestNewSolution(int id, Solution currentSolution, Task task, long timeout)
	{
		...
		Solution tempNewDeterministicSolution = CentralizedSolver.addTaskAndSearch(advCurrentSolutionMap.get(id),
		                                                                           task,
		                                                                           timeout - TIMEOUT_MARGIN - (System.currentTimeMillis() - startTime));
		
		if (System.currentTimeMillis() < startTime + timeout - 1000)
		{
			Solution tempNewSLSSolution = CentralizedSolver.slsSearch(
				vehiclesMap.get(id),
				newTaskSet,
		...
		{
			return tempNewDeterministicSolution;
		}
	}
	...
	public void auctionResult(Task task, int winner, Long[] bids) // timeout-bid
	{
		...
		if (!advCurrentTaskSetMap.containsKey(winner) && winner != agent.id())
		{
			...
			{
				if (System.currentTimeMillis() > startTime + bidTimeout - TIMEOUT_MARGIN)
					break;
				
				// Init so that the mapping of cities to the rival's vehicles is randomized
				initAgentMaps(winner);

				Solution advSolution = findBestNewSolution(winner,
				                                           advCurrentSolutionMap.get(winner),
				                                           task,
...
//		System.out.printf("[HTDAdv2] predictionCorrection: %s%n", predictionCorrection);
	...
	private Optional<Long> computeAdversarialMarginal(Task task, long timeout, long startTime)
	{
		...
		// TODO add boolean which accounts for this loop termination?
		
		long timeSpan = (timeout - TIMEOUT_MARGIN) / advCurrentTaskSetMap.entrySet().size();
		
		for (Map.Entry<Integer, Set<Task>> entry : advCurrentTaskSetMap.entrySet())
		{
			advTempNewSolutionMap.put(entry.getKey(),
			                          findBestNewSolution(entry.getKey(), advCurrentSolutionMap.get(entry.getKey()), task, timeSpan));
		}
	...
	private long computeBid(Optional<Long> adversarialMarginal)
	{
		...
		System.out.printf("[HTDAdv2] marginal cost == %s%n", lastMarginalCost);
		...
		if (lastMarginalCost <= 0)
		{
			...
			if (adversarialMarginal.isPresent())
			{
				...
				long attenuatedSafe = targetBid - loss;
				
				if (attenuatedAdvMarginal > attenuatedSafe)
					targetBid = (attenuatedAdvMarginal + attenuatedSafe) / 2;
				else
					targetBid = attenuatedAdvMarginal;
			...
			System.out.printf("[HTDAdv2] bid == %s%n", targetBid);
			return targetBid;
		}
		...
		if (newVisitedCities.size() == 0)
		{
			...
			System.out.printf("[HTDAdv2] no new city%n");
	...
	public Long askPrice(Task task) // timeout-bid
	{
		...
		Solution tempNewSolutionNeighbors = CentralizedSolver.addTaskAndSearch(currentSolution,
		                                                                       task,
		                                                                       bidTimeout - TIMEOUT_MARGIN - (System.currentTimeMillis() - startTime));
		Solution tempNewSolutionSLS = CentralizedSolver.slsSearch(vehicles,
		                                                          taskSet,
		                                                          (bidTimeout - 2 * TIMEOUT_MARGIN) - (System.currentTimeMillis() - startTime),
		                                                          startTime,
		                                                          random);
		...
		if (adversarialMarginal.isPresent())
		{
			System.out.printf("[HTDAdv2] adversarialMarginal: %s%n", adversarialMarginal);
//			adversarialMarginal = Optional.of(adversarialMarginal.get() + predictionCorrection);
	...
	public List<Plan> plan(List<Vehicle> vehicles, TaskSet tasks) // timeout-plan
	{
		...
		{
			System.out.printf("[HTDAdv2] empty solution%n");
			return currentSolution.getPlanList();
		}

		Solution finalSolution = CentralizedSolver.slsSearch(currentSolution,
		                                                     planTimeout - TIMEOUT_MARGIN,
		                                                     startTime,
		                                                     random);
		...
		System.out.printf("[HTDAdv2] adaptedFinalSolution cost: %d%n", adaptedFinalSolution.computeCost());
		System.out.printf("[HTDAdv2] totalRevenue: %d%n", totalRevenue);
		System.out.printf("[HTDAdv2] gain: %d%n", totalRevenue - adaptedFinalSolution.computeCost());
